...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        print("self.is_peft_model", self.is_peft_model)
    ...
    def _remove_unused_columns(self, dataset: "Dataset"):
        ...
        if "language_id" not in signature_columns:
            signature_columns += ["language_id"]
    ...
    def step(
        ...
        input_features = [tensor.to(self.current_device) for tensor in input_features] if input_features is not None else None

        feature_attention_masks = [tensor.to(self.current_device) for tensor in feature_attention_masks] if feature_attention_masks is not None else None
        ...
        if input_features is not None:
            # 如果传入的是 List，先堆叠成 Tensor [Batch, T, D]
            if isinstance(input_features, list):
                feats = torch.stack(input_features)
            else:
                feats = input_features
            
            # 确保在正确的设备上并放入字典
            model_inputs["input_features"] = feats.to(self.current_device)
        
        if feature_attention_masks is not None:
            if isinstance(feature_attention_masks, list):
                feature_attention_mask = torch.stack(feature_attention_masks)
            else:
                feature_attention_mask = feature_attention_masks
                
            model_inputs["feature_attention_mask"] = feature_attention_mask.to(self.current_device)
